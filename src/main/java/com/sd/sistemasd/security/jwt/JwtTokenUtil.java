...
public class JwtTokenUtil {
    ...
    public String generateAccesToken(EmpleadoBean user){
        return Jwts.builder()
                .setSubject(String.format("%s,%s", user.getEmpleadoID(), user.getEmail()))
    ...
    public boolean validateAccesToken(String token) {
        try {
            ...
            LOGGER.error("Error de firma. Token: {}", token, e);
        } catch (Exception e) {
            LOGGER.error("Error al validar el token. Token: {}", token, e);
        }
